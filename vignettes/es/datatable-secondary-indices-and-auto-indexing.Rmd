...
* [Francés](https://rdatatable.gitlab.io/data.table/articles/fr/datatable-secondary-indices-and-auto-indexing.html)
...
Esta viñeta supone que el lector está familiarizado con la sintaxis `[i, j, by]` de data.table y con la forma de realizar subconjuntos rápidos basados en claves. Si no estás familiarizado con estos conceptos, lea primero las viñetas [`vignette("datatable-intro", package="data.table")`](datatable-intro.html), [`vignette("datatable-reference-semantics", package="data.table")`](datatable-reference-semantics.html) y [`vignette("datatable-keys-fast-subset", package="data.table")`](datatable-keys-fast-subset.html).
...
* y finalmente obserar la *indexación automática* que va un paso más allá y crea índices secundarios automáticamente, pero lo hace en sintaxis nativa de R para subconjuntos.
...
* Ten en cuenta que `vuelos` **no** se reordena físicamente en orden creciente de `origen`, como habría sido el caso con `setkey()`.

* Ten en cuenta también que se ha añadido el atributo `index` a `flights`.
...
Considera el caso en el que desea realizar un subconjunto rápido basado en clave en la columna `origin` para el valor "JFK". Lo haríamos de la siguiente manera:
...
Y esto reordena los vuelos por "destino", *de nuevo*. Lo que realmente nos gustaría es poder realizar la subdivisión rápida eliminando el paso de reordenación.
...
* permite una sintaxis más clara al incluir las columnas en las que se ejecuta el subconjunto como parte de la sintaxis. Esto hace que el código sea más fácil de seguir cuando se lo analiza más adelante.
    ...
    Ten en cuenta que el argumento `on` también se puede utilizar en subconjuntos con claves. De hecho, te recomendamos proporcionar el argumento `on` incluso cuando se haga un subconjunto utilizando claves para mejorar la legibilidad.
# 
...
* Esta instrucción también realiza una búsqueda binaria rápida basada en subconjuntos, calculando el índice sobre la marcha. Sin embargo, ten en cuenta que no guarda el índice como un atributo automáticamente. Esto puede cambiar en el futuro.
...
Todas las operaciones que analizaremos a continuación no son diferentes a las que ya vimos en la viñeta [`vignette("datatable-keys-fast-subset", package="data.table")`](datatable-keys-fast-subset.html). Excepto que usaremos el argumento `on` en lugar de establecer claves.
...
Ya hemos visto este ejemplo en las viñetas [`vignette("datatable-reference-semantics", package="data.table")`](datatable-reference-semantics.html) y [`vignette("datatable-keys-fast-subset", package="data.table")`](datatable-keys-fast-subset.html). Echemos un vistazo a todas las `horas` disponibles en la tabla de datos `vuelos`:
...
Los demás argumentos, incluido `mult`, funcionan exactamente de la misma manera que vimos en la viñeta [`vignette("datatable-keys-fast-subset", package="data.table")`](datatable-keys-fast-subset.html). El valor predeterminado para `mult` es "all". Podemos elegir, pero solo se deben devolver las "primeras" o "últimas" filas coincidentes.
...
* Deshabilitar la indexación automática aún permite usar índices creados explícitamente con `setindex` o `setindexv`. Puedes deshabilitar los índices por completo configurando el argumento global `options(datatable.use.index = FALSE)`.
...
Discutiremos *subconjuntos* rápidos usando claves e índices secundarios para *uniones* en la [siguiente viñeta (`vignette("datatable-joins", package="data.table")`)](datatable-joins.html).
