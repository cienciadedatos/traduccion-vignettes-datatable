...
* [Francés](https://rdatatable.gitlab.io/data.table/articles/fr/datatable-reshape.html)
* [Ruso](https://rdatatable.gitlab.io/data.table/articles/ru/datatable-reshape.html)
...
Esta viñeta analiza el uso predeterminado de las funciones de remodelación `melt` (de ancho a largo) y `dcast` (de largo a ancho) para *data.tables*, así como las **nuevas funcionalidades extendidas** de *fusión* y *moldeo* [ingl. *melting* y *casting*] en *múltiples columnas* disponibles a partir de `v1.9.6`.
...
1. Primero, observar brevemente la conversión predeterminada de `melt` y `dcast` de `data.table` para convertirlas de formato *ancho* a *largo* y *viceversa*
...
### a) `data.table::melt` (de tabla ancha a larga)
...
* De manera predeterminada, la columna `variable` es del tipo `factor`. Establece el argumento `variable.factor` en `FALSO` si deseas devolver un vector de *`carácter`* en su lugar.

* De forma predeterminada, las columnas fundidas se denominan automáticamente `variable` y `valor`.
...
* Cuando no se especifican ni `id.vars` ni `measure.vars`, como se menciona en `?melt`, todas las columnas *no* `numéricas`, `enteras`, `lógicas` se asignarán a `id.vars`.
    ...
    Además, se emite un mensaje de advertencia destacando las columnas que se consideran automáticamente como `id.vars`.

### b) `data.table::dcast` (de largo a ancho)
...
## 2. Limitaciones de los enfoques anteriores de `melt` y `dcast`
...
Y deseas combinar (`melt`) todas las columnas `dob` y `gender`. Con la funcionalidad actual, podemos hacer algo como esto:
...
1. Lo que queríamos hacer era combinar todas las columnas de tipo `dob` y `gender` respectivamente. En lugar de eso, estamos combinando *todo* y luego dividiendo todo. Creo que es fácil ver que es bastante indirecto (e ineficiente).
    ...
    Como analogía, imagina que tienes un armario con cuatro estantes de ropa y te gustaría juntar la ropa de los estantes 1 y 2 (en el estante 1), y la de los estantes 3 y 4 (en el estante 3). Lo que estamos haciendo es, más o menos, juntar toda la ropa, y luego volver a dividirla en los estantes 1 y 3.

2. Las columnas que se van a "fundir" pueden ser de distintos tipos. Al "fundirlas" todas juntas, las columnas se convertirán en el resultado.

3. Estamos generando una columna adicional dividiendo la columna `variable` en dos columnas, cuyo propósito es bastante críptico. Lo hacemos porque lo necesitamos para el *moldeo* (`dcast`) en el siguiente paso.

4. Finalmente, *moldeamos* el conjunto de datos. Pero el problema es que es una operación que requiere mucho más trabajo computacional que *melt*. En concreto, requiere calcular el orden de las variables en la fórmula, y eso es costoso.
...
Como nos gustaría que `data.table` realice esta operación de manera sencilla y eficiente utilizando la misma interfaz, seguimos adelante e implementamos una *funcionalidad adicional*, donde podemos `fundir` varias columnas *simultáneamente*.
...
Los datos del iris tienen cuatro columnas numéricas con una estructura regular: primero la parte de la flor, luego un punto y luego la dimensión de la medida. Para especificar que queremos fundir esas cuatro columnas, podemos usar `measure` con `sep="."`, lo que significa usar `strsplit` en todos los nombres de columna; las columnas que resulten en la cantidad máxima de grupos después de la división se usarán como `measure.vars`:
...
¡Genial! Ahora podemos fundir varias columnas simultáneamente. Ahora, dado el conjunto de datos `DT.m2` como se muestra arriba, ¿cómo podemos volver al mismo formato que los datos originales con los que comenzamos?
