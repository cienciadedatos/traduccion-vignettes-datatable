...
* [Francés](https://rdatatable.gitlab.io/data.table/articles/fr/datatable-reference-semantics.html)
...
1. Primero, analizar brevemente la semántica de referencia y observemos las dos formas diferentes en las que se puede utilizar el operador `:=`

2. Luego veremos cómo podemos *agregar/actualizar/eliminar* columnas *por referencia* en `j` usando el operador `:=` y cómo combinarlo con `i` y `by`.
...
Antes de analizar la *semántica de referencia*, considera el *data.frame* que se muestra a continuación:
...
* En (a), `LHS` toma un vector de caracteres de nombres de columnas y `RHS` una *lista de valores*. `RHS` solo necesita ser `list`, independientemente de cómo se genere (por ejemplo, utilizando `lapply()`, `list()`, `mget()`, `mapply()`, etc.). Esta forma suele ser fácil de programar y es particularmente útil cuando no se conocen de antemano las columnas a las que se deben asignar valores.
...
En las dos formas de `:=` que se muestran arriba, tene en cuenta que no asignamos el resultado a una variable, porque no es necesario. La entrada *data.table* se modifica por referencia. Veamos algunos ejemplos para entender lo que queremos decir con esto.
...
#### -- Reemplaza aquellas filas donde `hora == 24` con el valor `0`
...
¿Cuál es la diferencia entre `flights[hour == 24L, hora := 0L]` y `flights[hour == 24L][, hour := 0L]`? Sugerencia: El último necesita una asignación (`<-`) si desea utilizar el resultado más adelante.
...
Ya hemos visto el uso de `i` junto con `:=` en la [Sección 2b](#ref-ij). Veamos ahora cómo podemos usar `:=` junto con `by`.
...
Limpiemos nuevamente y convirtamos nuestras columnas de factores recién creadas nuevamente en columnas de caracteres. Esta vez, haremos uso de `.SDcols` que acepta una función para decidir qué columnas incluir. En este caso, `is.factor()` devolverá las columnas que son factores. Para obtener más información sobre el **S**subconjunto de **D**ata, también hay una [viñeta de uso de SD](https://cran.r-project.org/package=data.table/vignettes/datatable-sd-usage.html).
...
`:=` modifica el objeto de entrada por referencia. Aparte de las características que ya hemos comentado, a veces podemos querer utilizar la función de actualización por referencia por su efecto secundario. Y en otras ocasiones puede que no sea deseable modificar el objeto original, en cuyo caso podemos utilizar la función `copy()`, como veremos en un momento.
...
* Tene en cuenta que se ha añadido la nueva columna `speed` a la tabla de datos `flights`. Esto se debe a que `:=` realiza operaciones por referencia. Dado que `DT` (el argumento de la función) y `flights` hacen referencia al mismo objeto en la memoria, la modificación de `DT` también se refleja en `flights`.
...
1. Contrariamente a la situación que hemos visto en el punto anterior, es posible que no queramos que la tabla de datos de entrada de una función se modifique *por referencia*. Como ejemplo, consideremos la tarea de la sección anterior, excepto que no queremos modificar `vuelos` por referencia.
    ...
    Primero, eliminemos la columna speed que generamos en la sección anterior.
    
    ```{r}
    flights[, speed := NULL]
    ```
    Ahora, podríamos realizar la tarea de la siguiente manera:
...
Sin embargo, podríamos mejorar esta funcionalidad aún más mediante una copia *superficial* en lugar de una copia *profunda*. De hecho, nos gustaría mucho [ofrecer esta funcionalidad para `v1.9.8`](https://github.com/Rdatatable/data.table/issues/617). Volveremos a abordar este tema en la viñeta *diseño de data.table*.
...
* También hemos visto cómo usar `:=` junto con `i` y `by` de la misma manera que hemos visto en la viñeta [`vignette("datatable-intro", package="data.table")`](datatable-intro.html). De la misma manera, podemos usar `keyby`, encadenar operaciones y pasar expresiones a `by` también de la misma manera. La sintaxis es *consistente*.
...
Hasta ahora hemos visto mucho sobre `j`, y cómo combinarlo con `by` y un poco de `i`. Volvamos nuestra atención a `i` en la [próxima viñeta (`vignette("datatable-keys-fast-subset", package="data.table")`)](datatable-keys-fast-subset.html) para realizar *subconjuntos ultrarrápidos* mediante *keying data.tables*.
