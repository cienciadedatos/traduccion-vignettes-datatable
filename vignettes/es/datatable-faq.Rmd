...
La primera sección, Preguntas frecuentes para principiantes, está pensada para leerse en orden, de principio a fin. Está escrita en un estilo de preguntas frecuentes para que se pueda digerir más fácilmente. En realidad, no son las preguntas más frecuentes. Una mejor medida para eso es buscar en Stack Overflow.

Estas preguntas frecuentes son de lectura obligatoria y se consideran documentación principal. No hagas preguntas en Stack Overflow ni plantees problemas en GitHub hasta que las hayas leído. Todos podemos darnos cuenta de que no las has leído cuando preguntas. Por lo tanto, si preguntas y no las has leído, no uses tu nombre real.
...
Por coherencia, de modo que cuando utilices data.table en funciones que aceptan entradas variables, puedas confiar en que `DT[...]` devuelva un data.table. No tienes que recordar incluir `drop=FALSE` como lo hace en data.frame. data.table se lanzó por primera vez en 2006 y esta diferencia con data.frame ha sido una característica desde el principio.

Sin embargo, es posible que hayas escuchado que, en general, es una mala práctica hacer referencia a las columnas por número en lugar de por nombre. Si tu colega viene y lee tu código más tarde, es posible que tengas que buscar por todas partes para encontrar qué columna es la número 5. Si tú o ellos cambian el orden de las columnas más arriba en tu programa R, puede producir resultados incorrectos sin advertencia o error si olvida cambiar todos los lugares en tu código que hacen referencia a la columna número 5. Eso es culpa suya, no de R ni de data.table. Es realmente muy malo. Por favor, no lo haga. Es el mismo mantra que tienen los desarrolladores profesionales de SQL: nunca use `select *`, siempre seleccione explícitamente por nombre de columna para al menos intentar ser robusto ante cambios futuros.
...
Una tercera propuesta más significativa que fue aceptada es que R ahora utiliza el código de ordenación por radix de data.table a partir de R 3.3.0:
...
*PERO* eso pasa por alto el punto principal. La mayoría de las tareas requieren que se haga algo con los datos después de una unión o fusión. ¿Por qué fusionar todas las columnas de datos, solo para usar un pequeño subconjunto de ellas después? Se podría sugerir `merge(X[ , ColsNeeded1], Y[ , ColsNeeded2])`, pero eso requiere que el programador determine qué columnas son necesarias. `X[Y, j]` en data.table hace todo eso en un solo paso para tú. Cuando escribe `X[Y, sum(foo*bar)]`, data.table inspecciona automáticamente la expresión `j` para ver qué columnas usa. Solo creará un subconjunto de esas columnas; las otras se ignoran. Solo se crea memoria para las columnas que usa `j` y las columnas `Y` disfrutan de las reglas de reciclaje estándar de R dentro del contexto de cada grupo. Digamos que `foo` está en `X` y `bar` está en `Y` (junto con otras 20 columnas en `Y`). ¿No es `X[Y, sum(foo*bar)]` más rápido de programar y más rápido de ejecutar que una `fusión` de todo seguida inútilmente por un `subconjunto`?
...
Por lo tanto, no haga, por ejemplo, `DT[ , sum(.SD[["sales"]]), by = grp]`. Eso funciona, pero es ineficiente y poco elegante. `DT[ , sum(sales), by = grp]` es lo que se pretendía, y podría ser cientos de veces más rápido. Si utiliza *todos* los datos en `.SD` para cada grupo (como en `DT[ , lapply(.SD, sum), by = grp]`), entonces ese es un muy buen uso de `.SD`. Si está utilizando *varias* pero no *todas* las columnas, puede combinar `.SD` con `.SDcols`; consulta `?data.table`.
...
Si quieres mostrar más de las 10 filas superiores (e inferiores) de un resumen de data.table (digamos que quieres 20), configure `options(datatable.print.topn = 20)`, por ejemplo. Nuevamente, también podría llamar a `print` directamente, como en `print(your.data.table, topn = 20)`.
...
La parte `Z[Y]` no es un nombre único, por lo que se evalúa dentro del marco de `X` y se produce el problema. Prueba `tmp = Z[Y]; X[tmp]`. Esto es robusto para `X` que contiene una columna `"tmp"` porque `tmp` es un nombre único. Si a menudo encuentras conflictos de este tipo, una solución sencilla puede ser nombrar todas las tablas en mayúsculas y todos los nombres de columnas en minúsculas, o algún esquema similar.
...
Si ya está ejecutando v1.8.1 o posterior, entonces el mensaje de error ahora es más útil que el error "no se puede cambiar el valor del enlace bloqueado", como puedes ver arriba, ya que esta viñeta se produjo usando v1.8.1 o posterior.
