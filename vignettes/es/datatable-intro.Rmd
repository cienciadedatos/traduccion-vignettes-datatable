...
* [Francés](https://rdatatable.gitlab.io/data.table/articles/fr/datatable-intro.html)
* [Ruso](https://rdatatable.gitlab.io/data.table/articles/ru/datatable-intro.html)
...
* Sintaxis *concisa* y *consistente* independientemente del conjunto de operaciones que desees realizar para lograr su objetivo final.
...
En resumen, si estás interesado en reducir enormemente el tiempo de *programación* y *computación*, este paquete es para tí. La filosofía a la que se adhiere `data.table` lo hace posible. Nuestro objetivo es ilustrarte a través de esta serie de viñetas.
...
Toma `DT`, subconjunto/reordene filas usando `i`, luego calcula `j`, agrupado por `by`.

Comencemos mirando primero `i` y `j`: subconjuntando filas y operando en columnas.

### c) Subconjunto de filas en `i` {#subset-i-1c}

#### -- Obtén todos los vuelos con "JFK" como aeropuerto de origen en el mes de junio.
...
#### -- Obtener las dos primeras filas de `flights`. {#subset-rows-integer}
...
#### -- Ordena `flights` primero por la columna `origin` en orden *ascendente*, y luego por `dest` en orden *descendente*:
...
* Podemos usar "-" en columnas de `caracteres` dentro del marco de una `data.table` para ordenar en orden decreciente.

* Además, `order(...)` dentro del marco de una `data.table` utiliza el ordenamiento rápido interno de `data.table` `forder()`. Esta ordenación proporcionó una mejora tan convincente con respecto a `base::order` de R que el proyecto R adoptó el algoritmo `data.table` como su ordenamiento predeterminado en 2016 para R 3.3.0 (para referencia, consulta `?sort` y las [NOTICIAS de la versión R](https://cran.r-project.org/doc/manuals/r-release/NEWS.pdf)).

Discutiremos el orden rápido de `data.table` con más detalle en la viñeta *Aspectos internos de `data.table`*.

### d) Selecciona la(s) columna(s) en `j` {#select-j-1d}
...
* Dado que se puede hacer referencia a las columnas como si fueran variables dentro del marco de una `data.table`, hacemos referencia directamente a la *variable* sobre la que queremos un subconjunto. Como queremos *todas las filas*, simplemente omitimos `i`.

* Devuelve *todas* las filas de la columna `arr_delay`.

#### -- Selecciona la columna `arr_delay`, pero devuélvala como `data.table` en su lugar.
...
Una tabla `data.table` (y también un `data.frame`) es internamente una lista (`list`) también, con la condición de que cada elemento tiene la misma longitud y la lista tiene un atributo `class`. Al permitir que `j` devuelva una `lista`, se oyede convertir y devolver una `data.table` de manera muy eficiente.
...
#### -- Selecciona las columnas `arr_delay` y `dep_delay`.
...
#### -- Selecciona las columnas `arr_delay` y `dep_delay` *y* cambiale el nombre a `delay_arr` y `delay_dep`.
...
* Primero creamos un subconjunto en `i` para encontrar los *índices de fila* coincidentes donde el aeropuerto `origin` es igual a `"JFK"`, y el mes es igual a `6L`. *Todavía* no creamos un subconjunto de la tabla de datos *completa* correspondiente a esas filas.
...
Debido a que los tres componentes principales de la consulta (`i`, `j` y `by`) están *juntos* dentro de `[...]`, `data.table` puede ver los tres y optimizar la consulta en conjunto *antes de la evaluación*, en lugar de optimizar cada uno por separado. Por lo tanto, podemos evitar el subconjunto completo (es decir, crear subconjuntos de las columnas *además de* `arr_delay` y `dep_delay`), tanto por velocidad como por eficiencia de memoria.
...
Para coincidencias exactas sin `NA` para elementos inexistentes, utiliza `nomatch=NULL`:
...
`.N` es una variable incorporada especial que contiene la cantidad de observaciones *en el grupo actual*. Es particularmente útil cuando se combina con `by` como veremos en la siguiente sección. En ausencia de operaciones de agrupamiento por, simplemente devuelve la cantidad de filas en el subconjunto.
...
* Ten en cuenta que no hemos incluido `.N` en `list()` o `.()`. Por lo tanto, se devuelve un vector.
...
### h) ¡Genial! Pero, ¿cómo puedo hacer referencia a columnas por nombres en `j` (como en un `data.frame`)? {#refer-j}

Si escribe los nombres de las columnas explícitamente, no hay diferencia en comparación con un `data.frame` (desde v1.9.8).

#### -- Selecciona las columnas `arr_delay` y `dep_delay` mediante el método `data.frame`.
...
Si has almacenado las columnas deseadas en un vector de caracteres, hay dos opciones: utilizar el prefijo `..` o utilizar el argumento `with`.
...
El argumento se llama `with` en honor a la función R `with()` debido a que su funcionalidad es similar. Supongamos que tienes un `data.frame` `DF` y deseas crear un subconjunto de todas las filas donde `x > 1`. En `base` R puede hacer lo siguiente:
...
* El uso de `with()` en (2) permite usar la columna `x` de `DF` como si fuera una variable.
    ...
    De aquí el nombre del argumento con nombre `with` en `data.table`. Establecer `with = FALSE` desactiva la capacidad de referirse a las columnas como si fueran variables, restaurando así el modo `data.frame`.
...
* Desde `v1.9.5+`, también podemos seleccionar especificando los nombres de las columnas de inicio y fin, por ejemplo, `año:día` para seleccionar las primeras tres columnas.
    ...
    Esto es especialmente útil cuando se trabaja de manera interactiva.
...
* Sabemos que `.N` [es una variable especial](#special-N) que contiene la cantidad de filas del grupo actual. Al agrupar por `origen` se obtiene la cantidad de filas, `.N`, para cada grupo.

* Al ejecutar `head(flights)`, puedes ver que los aeropuertos de origen aparecen en el orden *"JFK"*, *"LGA"* y *"EWR"*. El orden original de agrupación de las variables se conserva en el resultado. *¡Es importante tener esto en cuenta!*
...
* Al utilizar esos *índices de fila*, obtenemos la cantidad de filas agrupadas por `origin`. Una vez más, aquí no se materializan columnas, ya que la `j-expression` no requiere que se creen subconjuntos de ninguna columna y, por lo tanto, es rápida y eficiente en el uso de la memoria.
...
* Una vez más, Ten en cuenta que el orden de entrada de las columnas de agrupación se conserva en el resultado.

¿Y ahora qué pasa si queremos ordenar el resultado por las columnas de agrupación `origin`, `dest` y `month`?
...
Aprenderemos más sobre `keys` en la viñeta [`vignette("datatable-keys-fast-subset", package="data.table")`](datatable-keys-fast-subset.html); por ahora, todo lo que tienes que saber es que puedes usar `keyby` para ordenar automáticamente el resultado por las columnas especificadas en `by`.
...
* Recordemos que podemos usar `-` en una columna de caracteres en `order()` dentro del marco de un `data.table`. Esto es posible gracias a la optimización de consultas internas de `data.table`.

* Recuerda también que `order(...)` con el marco de una `data.table` se *optimiza automáticamente* para usar el orden de radio rápido interno de `data.table` `forder()` para mayor velocidad. 
...
* Ten en cuenta que no proporcionamos ningún nombre a `by-expression`. Por lo tanto, los nombres se han asignado automáticamente en el resultado. Al igual que con `j`, puede nombrar estas expresiones como lo haría para los elementos de cualquier `lista`, como por ejemplo `DT[, .N, .(dep_delayed = dep_delay>0, arr_delayed = arr_delay>0)]`.

* Puedes proporcionar otras columnas junto con expresiones, por ejemplo: `DT[, .N, by = .(a, b>0)]`.
...
¿Cómo podemos hacer esto de manera eficiente y concisa? Para lograrlo, repasemos [este consejo](#tip-1): *«Siempre que la expresión `j` devuelva una `lista`, cada elemento de la `list` se convertirá en una columna en la `data.table` resultante»*. Si podemos hacer referencia al *subconjunto de datos de cada grupo* como una variable *mientras agrupamos*, podemos recorrer todas las columnas de esa variable utilizando la función base `lapply()`, que ya nos resulta familiar o que pronto nos resultará familiar. No hay que aprender nuevos nombres específicos de `data.table`.
...
Recuerda que una `data.table` es internamente también una `lista` con todas sus columnas de igual longitud.
...
De manera similar a [parte g)](#refer-j), también puedes especificar las columnas que deseas eliminar en lugar de las columnas que deseas conservar utilizando `-` o `!`. Además, puede seleccionar columnas consecutivas como `colA:colB` y deseleccionarlas como `!(colA:colB)` o `-(colA:colB)`.
...
* Eso es todo. No se requiere ninguna sintaxis especial. Todo lo que necesitamos saber es la función base `c()` que concatena vectores y [la sugerencia de antes](#tip-1).
...
* Ten en cuenta que esas comas son solo para visualización. Una columna de lista puede contener cualquier objeto en cada celda y, en este ejemplo, cada celda es en sí misma un vector y algunas celdas contienen vectores más largos que otras.
...
La flexibilidad de j nos permite almacenar cualquier objeto de lista como un elemento de data.table. Por ejemplo, cuando los modelos estadísticos se ajustan a grupos, estos modelos se pueden almacenar en una tabla data.table. El código es conciso y fácil de entender.
...
Podemos hacer mucho más en `i` si introducimos claves en `data.table`, lo que permite crear subconjuntos y uniones increíblemente rápidos. Veremos esto en las viñetas [`vignette("datatable-keys-fast-subset", package="data.table")`](datatable-keys-fast-subset.html) y [`vignette("datatable-joins", package="data.table")`](datatable-joins.html).

#### Usando `j`:

1. Selecciona columnas con el método `data.table`: `DT[, .(colA, colB)]`.

2. Selecciona columnas con el método `data.frame`: `DT[, c("colA", "colB")]`.

3. Calcular en las columnas: `DT[, .(sum(colA), mean(colB))]`.

4. Proporciona nombres si es necesario: `DT[, .(sA = suma(colA), mB = media(colB))]`.

5. Combina con `i`: `DT[colA > valor, suma(colB)]`.
