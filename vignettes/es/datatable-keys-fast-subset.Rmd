...
* [Francés](https://rdatatable.gitlab.io/data.table/articles/fr/datatable-keys-fast-subset.html)
...
* primero, introducir el concepto de `clave` en *data.table*, y establecer y usar claves para realizar *búsquedas binarias rápidas* basadas en subconjuntos en `i`,
...
* y finalmente concluimos mirando la ventaja de configurar claves: realizar *búsquedas binarias rápidas basadas en subconjuntos* y comparar con el enfoque de escaneo vectorial tradicional.

## 1. Llaves

### a) ¿Qué es una *clave*?

En la viñeta [`vignette("datatable-intro", package="data.table")`](datatable-intro.html), vimos cómo crear subconjuntos de filas en `i` usando expresiones lógicas, números de fila y el uso de `order()`. En esta sección, veremos otra forma de crear subconjuntos increíblemente rápido: usando *claves*.

Pero primero, comencemos por analizar los *data.frames*. Todos los *data.frames* tienen un atributo de nombre de fila. Considera el *data.frame* `DF` que se muestra a continuación.
...
1. Cada fila está limitada a *exactamente un* nombre de fila.
    ...
    Pero, una persona (por ejemplo) tiene al menos dos nombres: - uno *primero* y un *segundo*. Esto es útil para organizar una guía telefónica por *apellido* y luego por su *primer nombre*.
...
* Ten en cuenta que los nombres de las filas se han restablecido.

* *data.tables* nunca utilices nombres de fila. Dado que *data.tables* **hereda** de *data.frames*, aún tiene el atributo de nombres de fila, pero nunca los utiliza. En un momento veremos por qué.

    Si deseas conservar los nombres de las filas, usa `keep.rownames = TRUE` en `as.data.table()` - Esto creará una nueva columna llamada`rn` y asignará los nombres de las filas a esta columna.

En cambio, en *data.tables*, establecemos y usamos `keys`. Pensa en una `key` como si fuera un conjunto de **nombres de fila supercargados**.
...
3. Establecer una `clave` hace *dos* cosas:
    ...
    a. Reordena físicamente las filas del  *data.table* por las columna(s) proporcionada(s) *por refefencia*, siempre en orden *creciente*.
    
    b. Marca esas columnas como columnas *keys* al establecer un atributo llamado `sorted` en el *data.table*.
    
    Dado que las filas se reordenan, un *data.table* puede tener como máximo una clave, ya que no se puede ordenar de más de una manera.
...
* Puedes utilizar la función `setkey()` y proporcionar los nombres de las columnas (sin comillas). Esto resulta útil durante el uso interactivo.

* Alternativamente, puedes pasar un vector de caracteres de nombres de columnas a la función `setkeyv()`. Esto es particularmente útil al diseñar funciones para pasar columnas a las que se les asignará una clave como argumentos de función.

* Ten en cuenta que no tuvimos que asignar el resultado a una variable. Esto se debe a que, al igual que la función `:=` que vimos en la viñeta [`vignette("datatable-reference-semantics", package="data.table")`](datatable-reference-semantics.html), `setkey()` y `setkeyv()` modifican la entrada *data.table* *por referencia*. Devuelven el resultado de forma invisible.

* *data.table* ahora está reordenada (u ordenada) por la columna que proporcionamos: `origin`. Como reordenamos por referencia, solo necesitamos memoria adicional de una columna de longitud igual a la cantidad de filas en la *data.table* y, por lo tanto, es muy eficiente en el uso de la memoria.

* También puedes establecer claves directamente al crear *data.tables* utilizando la función `data.table()` con el argumento `key`. Toma un vector de caracteres de nombres de columnas.
...
Una vez que se *identifica* una *data.table* por determinadas columnas, se puede crear un subconjunto consultando esas columnas clave utilizando la notación `.()` en `i`. Recuerda que `.()` es un *alias* de `list()`.

#### -- Utiliza la columna clave `origen` para crear un subconjunto de todas las filas donde el aeropuerto de origen coincida con *"JFK"*
...
* Podemos crear subconjuntos de cualquier cantidad de valores según sea necesario
    ...
    Esto devuelve todas las columnas correspondientes a aquellas filas donde la columna origin coincide con *"JFK"* or *"LGA"*.
...
* Es importante entender cómo funciona esto internamente. *"JFK"* se compara primero con la primera columna de clave `origin`. Y *dentro de esas filas coincidentes*, *"MIA"* se compara con la segunda columna de clave `dest` para obtener *índices de fila* donde tanto `origin` como `dest` coinciden con los valores dados.
...
* Una vez que tenemos los índices de fila, observamos `j`, que requiere solo la columna `arr_delay`. Por lo tanto, simplemente seleccionamos la columna `arr_delay` para esos *índices de fila* de la misma manera que hemos visto en la viñeta [`vignette("datatable-intro", package="data.table")`](datatable-intro.html).
...
#### -- Sobre el resultado obtenido anteriormente, utiliza el encadenamiento (chaining) para ordenar la columna en orden decreciente.
...
Ya hemos visto este ejemplo en la viñeta [`vignette("datatable-reference-semantics", package="data.table")`](datatable-reference-semantics.html). Echemos un vistazo a todas las `horas` disponibles en la *tabla de datos* `vuelos`:
...
* Ahora podemos crear un subconjunto de `hora` utilizando la notación `.()`. Creamos un subconjunto para el valor *24* y obtenemos los *índices de fila* correspondientes.
...
* Creamos un subconjunto de la columna `clave` *origin* para obtener los *índices de fila* correspondientes a *"JFK"*.
...
* Una vez más, la consulta para la segunda columna clave `dest`, *"XNA"*, se recicla para ajustarse a la longitud de la consulta para la primera columna clave `origin`, que tiene una longitud 3.
...
Una de las ventajas más probables es que la sintaxis es más corta. Pero, además, los *subconjuntos basados en búsquedas binarias* son **increíblemente rápidos**.

A medida que pasa el tiempo, `data.table` obtiene una nueva optimización y actualmente la última llamada se optimiza automáticamente para usar *búsqueda binaria*. Para usar *escaneo vectorial* lento, se debe eliminar la clave.
...
Ahora veamos el enfoque de búsqueda binaria (método 2). Recordemos de [Propiedades de clave](#key-properties): *establecer claves reordena la tabla de datos por columnas de clave*. Dado que los datos están ordenados, no tenemos que *escanear a través de toda la longitud de la columna*. En cambio, podemos usar *búsqueda binaria* para buscar un valor en `O(log n)` en lugar de `O(n)` en el caso del *enfoque de escaneo vectorial*, donde `n` es el número de filas en la *tabla de datos*.
...
Los subconjuntos basados en claves son **increíblemente rápidos** y son particularmente útiles cuando la tarea implica *subconjuntos repetidos*. Pero puede que no siempre sea deseable establecer la clave y reordenar físicamente la *tabla de datos*. En la siguiente [próxima viñeta (`vignette("datatable-secondary-indices-and-auto-indexing", package="data.table")`)](datatable-secondary-indices-and-auto-indexing.html), abordaremos este problema utilizando una *nueva* característica: *índices secundarios*.
