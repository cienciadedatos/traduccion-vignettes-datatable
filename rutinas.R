...
extraer_traducciones_con_selenium <- function(files, files_es, wait = .8) {
  while(TRUE) {
    ...
    stop(errorCondition(
      "No fue posible iniciar Selenium", 
...
extraer_traducciones_con_chromote <- function(files, files_es, wait = .8) {
  tryCatch(
    ...
    error = \(e) {
      stop(errorCondition(
        sprintf("No fue posible iniciar Selenium: %s", conditionMessage(e)), 
        class = "web.driver.error",
        call = sys.call()))
    }
  )
...

{
  ...
  tryCatch(
    ...
    error = \(e) stop(conditionMessage(e)),
    {
      ...
      attempt <- 0
      cat("Debe actualizar el repo subiendo los .txt en inglés a ../google-translations\n")
      cat("Luego se intentará traducir los txt desde la web, usando google\n")
      cat("El script puede actualizar el repo por usted\n")
      cat("Si no actualiza el repo, la traducción se hará con los mensajes del repo, que pueden estar obsoletos\n")
      while((attempt <- attempt + 1L) <= 3) {
        if (inherits(git_version, "try-error") && !nzchar(PAT) || attempt > 1L) {
          cat("NOTA: Probablemente deba generar un Personal Access Token de Github\n")
        }
        switch(
          menu(title= "Elija una opción", c(
            sprintf("Intentar `git pull` desde R %s", pat_alert),  
            "Generar y/o copiar un token PAT via github",
            "Continuar sin actualizar el repo (o actualizarlo manualmente y continuar")), 
          {
            NULL
          },{
        ...
        tryCatch(
          ...
          error = \(e) {
            ...
            if (attempt < 3) message(sprintf("Nuevo intento (%d)...", attempt + 1L))
              ...
              cat("Debe actualizar manualmente el repo subiendo los .txt en inglés a ../google-translations\n")
              cat("Si no actualiza el repo, la traducción se hará con los mensajes del repo, que pueden estar obsoletos\n")
